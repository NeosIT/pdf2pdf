...
function pdf2pdf(options, callback) {
    ...
    convert.convert({gm_bin: gm_bin, in_pdf: infile, temp_path: tmpdir}, options.cb_status, function (error, file_list) { //run pdf to tiff conversion
        if (error)
            callback(error)
        else {
            ocr.run_ocr(tesseract, file_list, tmpdir, options.cb_status, function (error, hocr_html, tmp_img) { //run tesseract
                if (error)
                    callback(error)
                else {
                    parser.generatePDF(tmp_img, hocr_html, tmpdir, options.outfile, options.cb_status, function (error) { //parse tesseract output and generate searchable PDF
                        if (error)
                            callback(error)
                        else {
                            //cleanup
                            for (i in tmp_img){
                                fs.unlinkSync(tmp_img[i])
                                fs.unlinkSync(tmp_img[i]+'.jpg')
                                fs.unlinkSync(tmp_img[i]+'.html')
                            }
