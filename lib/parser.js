...
function writePDF(input_img, input_html, temp_path, output_pdf, res_images, i, doc, cb_status, callback){
    if (i == input_img.length){
        ...
        gm(input_img[i]).size(function (err, size) { //get the image dimensions
            size.width = size.width / res_images[i][0] * 72
            size.height = size.height / res_images[i][1] * 72
			...
			if (max_words != 0)
			{
                        html_data('.ocrx_word').each(function (idx_word, item_word) { //parse current page and process each word individually
                            ...
                            var x1 = parseInt(item_word.attribs.title.split(';')[0].split(' ')[1], 10) / res_images[i][0] * 72//get bounding box coordinates
                            var x2 = parseInt(item_word.attribs.title.split(';')[0].split(' ')[3], 10) / res_images[i][1] * 72
                            var y1 = parseInt(item_word.attribs.title.split(';')[0].split(' ')[2], 10) / res_images[i][0] * 72
                            var y2 = parseInt(item_word.attribs.title.split(';')[0].split(' ')[4], 10) / res_images[i][1] * 72
                            ...
                            if (idx_word == max_words - 1) { //check for last word
                                doc.image(input_img[i] + '.jpg', 0, 0, {width: size.width, height: size.height}) //place image over text
                                writePDF(input_img, input_html, temp_path, output_pdf, res_images, ++i, doc, cb_status, callback)
                            }
                        })
                        }
                        else
...
function generatePDF(input_img, input_html, temp_path, output_pdf, res_images, cb_status, callback) {
    gm(input_img[0]).size(function (err, size) { //get the image dimensions
        size.width = size.width / res_images[0][0] * 72
        size.height = size.height / res_images[0][1] * 72
        var doc = new pdfkit({size: [size.width, size.height]}) //create PDF with determined size
        writePDF(input_img, input_html, temp_path, output_pdf, res_images, 0, doc, cb_status, function (err){
            callback (err)
        })
    })
}
