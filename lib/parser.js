...
function generatePDF(input_tiff, input_html, temp_path, output_pdf, callback) {
    ...
    fs.readFile(input_html, function (err, data) { //read tesseract-ocr output
        if (err) {
            ...
            parse_html(cheerio.load(data), input_tiff, output_pdf, [temp_path, '/ocr-', crypto.randomBytes(6).readUInt32LE(0)].join(''), callback); //run the parser
...
function parse_html(html_data, in_tiff, out_pdf, tmp, callback) {
